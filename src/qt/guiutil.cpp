...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("kurdcoin"))
        return false;
...
{
    // Convert kurdcoin:// to kurdcoin:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("kurdcoin://"))
    {
        uri.replace(0, 12, "kurdcoin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "KurdCoin.lnk";
}
...
{
    return GetAutostartDir() / "kurdcoin.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("KurdCoin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  kurdcoin-qt [" + tr("command-line options") + "]                     " + "\n";
...
void SetBlackThemeQSS(QApplication& app)
{
    app.setStyleSheet("QWidget        { background: rgb(41,44,48); }"
                      ...
                      "QMenuBar       { padding-left: 10px; background: rgb(41,44,48); color: rgb(110,116,126); }"
                      "QMenuBar::item { background: transparent; color: rgb(110,116,126); }"
                      ...
                      "QToolButton    { background: rgb(30,32,36); color: rgb(116,122,134); border: none; border-left-color: rgb(30,32,36); border-left-style: solid; border-left-width: 6px; margin-top: 2px; margin-bottom: 2px; }"
